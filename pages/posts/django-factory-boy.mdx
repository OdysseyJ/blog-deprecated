...
# Python에서 Factory boy 를 사용해 인스턴스 제작

## Factory boy가 뭔가요?

- 인스턴스를 쉽게 생성할 수 있도록 도와주는 패키지입니다.

## 버전
- factory-boy 2.12.0을 기준으로 작성했습니다.
```
...
- 팩토리는 두가지의 생성 전략을 제공한다.
    ...
    <Callout type="warning" emoji="⚠️">
        ...
        2.0 이상부터는 Modle(**kwargs)를 호출한다. Django model에 관해서는 DjangoModelFactory 클래스를 사용해야 한다.
    </Callout>
...
## 기본 클래스 함수들
    ...
    - _adjust_kwargs(cls, **kwargs)
        - late field 튜닝을 가능하게 해 줍니다. 아래의 코드는 lastname이 항상
          uppercase임을 보장합니다.
            ```
            class UserFactory(factory.Factory):
            ...
            ```
    - _build, _create
        - build, create가 일어나기 전에 실행되는 함수입니다.

    - reset_sequence(cls, value=None, force=False)
        - 해당 팩토리가 가진 시퀀스를 초기화해줍니다.
...
## Declaration
    ...
    ### SelfAttribute
        - 다른 Attribute를 참조해 Attribute를 생성해야 하는 경우에 사용합니다.
            ```
            class UserFactory(factory.Factory):
                class Meta:
                    model = User
    ...
    ### Maybe
        - 만약, 특정 다른 어트리뷰트에 따라서 field를 build하는 케이스가 나눠져야
          할 경우 Maybe를 사용한다.
          ```
        class UserFactory(factory.Factory):
            class Meta:
